...
{*/                                                                         }
{//-------------------------------------------------------------------------}
{*** THE LOST ADVENTURES OF KROZ player movement.  By Scott Miller 11/12/89 ***}
...
uses CRT, DOS, Lost1D, Lost2D, Lost2BD, Lost3D, Lost4D;
...
procedure Tablet_Message(Level: integer);
 begin
  case Level of
   ...
   8: Flash(10,25,'Play this level as if viewing it from the side.');
   9: Flash(10,25,'Explorer, you fell into a trap with no way out!');
   10:Flash(3,25,'A magical forest grows out of control in this region of Kroz!');
   11:Flash(14,25,'The tunnel below contains a magic spell.');
   18:Flash(14,25,'Adventurer, check the "Valley of the M"!');
   22:Flash(16,25,'These walls will seek to entrap you!');
   24:Flash(12,25,'You have choosen the greedy path Adventurer!');
   26:begin
       Flash(13,25,'No one has ever made it to the 26th level!');
       Flash(7,25,'You have shown exceptional skills to reach this far...');
       Flash(13,25,'Therefore I grant you the power to see...');
       bak(0,0);
       ...
       Flash(20,25,'Behold...your path awaits...');
      end;
   30:begin
       Prayer;
   ...
   32:begin
       Flash(14,25,'Adventurer, your path will be cleared...');
       PF[PX,PY]:=44;
       Move(0,0,GMove);
      end; 
   37:begin
       Prayer;
       Flash(4,25,'"Useless bits of rock, rubble and boulders, shall become..."');
       bak(0,0);
       for x := XBot to XTop do
        for y := YBot to YTop do
         if PF[x,y] = 65 then
       ...
       Flash(20,25,'"...Precious Gems of value!"');
      end;
   40:Flash(5,25,'You''ll need two keys from level 38 to complete this level!');
   42:begin
       Prayer;
   ...
   46:Flash(9,25,'Follow the sequence if you wish to be successful.');
   48:begin
       Prayer;
       Flash(9,25,'"A gift can be granted to those who are worthy..."');
       bak(0,0);
       for x := XBot to XTop do
        for y := YBot to YTop do
         if PF[x,y] = 55 then
       ...
       Flash(13,25,'"...And you are found worthy, Adventurer!"');
      end;
   52:Flash(18,25,'Up 4 steps, then left 16 steps.');
   54:Flash(24,25,'K stands from Kroz!');
   56:begin
       bak(0,0);
       for x := XBot to XTop do
        for y := YBot to YTop do
       ...
       Flash(17,25,'The Lava parts to give you access!');
      end;
   61:begin
       Flash(9,25,'Walls that block your progress shall be removed...');
       PF[PX,PY]:=58;
       Move(0,0,GMove);
      end; 
   64:begin
       Prayer;
       Flash(10,25,'"Tnarg yna rerutnevda ohw sevivrus siht raf..."');
       bak(0,0);
       for x := XBot to XTop do
        for y := YBot to YTop do
         if PF[x,y] = 55 then
   ...
   66:Flash(5,25,'"Figurit''s Revenge" appeared in the popular game Jumpman!');
   68:begin
       Flash(11,25,'Suddendly, the lava begins to bubble and flow!');
       LavaFlow:=true;LavaRate:=75;
      end;
   71:begin
       Prayer;
       Flash(19,25,'"Those not blinded by hate..."');
       bak(0,0);
       for x := XBot to XTop do
        for y := YBot to YTop do
         if PF[x,y] = 4 then
       ...
       Flash(20,25,'"...will be allowed to see!"');
      end;
   72:begin
       Flash(6,25,'You feel the entire chamber shiver for a brief moment...');
       Flash(9,25,'then a message magically appears on the tablet...');
       Flash(3,25,'"Find the Sacred Tome of Kroz, use it''s wisdom to free us..."');
       for b:= 1 to 25000 do sound(random(5000)+3000);nosound;
       Flash(12,25,'The the tablet disentagrates in your hands!');
       Flash(9,25,'You wonder for a moment what the message meant...');
       Flash(6,25,'And resume you journey no more enlightened than before.');
      end;
   74:begin
       EvapoRate:=30;
       Flash(4,25,'Suddenly an intense heat sweeps over this entire chamber...');
       Flash(5,25,'You begin to sweat profusely and drink some of your water!');
      end;
   75:begin
       Prayer;
       Flash(18,25,'"Ttocs Rellim Setalutargnoc Uoy!"');
       Flash(5,25,'Your palms sweat as the words echo through the chamber...');
       bak(0,0);
       for x := XBot to XTop do
        for y := YBot to YTop do
         if PF[x,y] = 23 then
       ...
       Flash(16,25,'...Your eyes widen with anticipation!');
      end;
  end;
 end; { Tablet_Message }
...
procedure Next_Level;
 begin
  case Level of  { 41 pre-defined levels, 34 random levels }
   1:Level1;
   2:Level2;
   4:Level4;
   6:Level6;
 ...
 begin
   ...
   {SpeedTime}15:begin
                  ...
                  if not(15 in FoundSet) then
                   begin
                    ...
                    Flash(18,25,'You activated a Speed Creature spell.');
                   end;
                 end;
   {Trap}     16:begin
                  Go(XWay,YWay,Human);
  ...
  {Forest/Tree} 19,20:if Human then
   ...
   {Tome}     24:begin
                  ...
                  for x:=1 to 24 do
                   for y:=5 downto 1 do
                    begin
                     ...
                     gotoxy(33,8);col(random(16),random(16));
                     write(Tome);
                    end;
                  gotoxy(33,8);
                  col(16,16);bak(2,7);
                  ...
                  Update_Info;
                  ClearKeys;
                  Flash(6,25,'The Sacred Tome of Kroz is finally yours--50,000 points!');
  ...
  {WallVanish}47:begin
   ...
   {Triggers} 68..74:begin
                  ...
                  case a of
                   ...
                   73: begin i:=27;col(ArtColor,7);ch:=Nugget;end;
